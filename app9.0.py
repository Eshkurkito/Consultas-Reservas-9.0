...
# Utilidades: descarga del código fuente
with st.sidebar.expander("🔧 Utilidades"):
    try:
        import inspect, sys
        _src = inspect.getsource(sys.modules[__name__])
        st.download_button("📥 Descargar app.py", data=_src, file_name="app.py", mime="text/x-python")
    except Exception as e:
        st.caption(f"No se pudo preparar la descarga del código: {e}")
